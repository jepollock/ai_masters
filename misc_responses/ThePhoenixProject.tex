...
In that equation, "wait time" has no units. It isn't "wait time", it can't be, there aren't any units attached to it. Constraints might appear by sorting, but that's not what it's described as.
...
This is from pipelining. Work that flows upstream will cause pipeline stalls. If the pipeline is deep, this gets extremely expensive as everyone stands around waiting.
...
The cost of a bad release is related to the square of both the days in the release and the number of developers.

Reducing the size of the release by a factor of 10 (one 10th the size) will reduce our the cost of a bad release by a factor of 100. Reducing the error rate by a factor of 10 (one 10th the number of bugs) only reduces the costs by a factor of 10.
