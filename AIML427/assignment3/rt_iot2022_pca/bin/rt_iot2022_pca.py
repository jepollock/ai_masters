...
def main():
    if len(sys.argv) != 4:
        print("Usage: rt_iot2022_pca.py <num_runs> <input_file> <output_dir>")
        sys.exit(-1)
    ...
    spark = SparkSession.builder.appName("rt_iot2022_pca").getOrCreate()
    ...
    # get the count
    # get the frequencies of the targets.
    #
    ...
    debug(feature_columns)
    indexed_df = label_indexer.fit(raw_dataframe).transform(raw_dataframe)
    debug(indexed_df)
    ...
    feature_vector_pca = PCA(k=3, inputCol=SCALED_FEATURE_VECTOR_COLUMN, outputCol=PCA_FEATURE_VECTOR_COLUMN)

    decision_tree = DecisionTreeClassifier(labelCol=LABEL_COLUMN, featuresCol=PCA_FEATURE_VECTOR_COLUMN)
    pipeline_stages = []
    pipeline_stages.append(label_indexer)
    pipeline_stages.extend(likelihood_encoders)
    pipeline_stages.extend([feature_vector_assembler, feature_vector_scaler, feature_vector_pca, decision_tree])
    ...
    # JASON: TODO - for each model, extract the primary component and dump them.
    ...
    data = [
        ( "pca_test", float(numpy.average(accuracies[:, 1])), float(numpy.std(accuracies[:, 1]))),
        ("pca_train", float(numpy.average(accuracies[:, 0])), float(numpy.std(accuracies[:, 0])))
    ]
